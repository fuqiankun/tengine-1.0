...
void dump_mem_prof(void) {
  ...
  while (fgets(line, 128, fp)) {
    if (line[0] == 'V' && line[1] == 'm') LOG_INFO() << line;
  }
...
void release_tengine(void) {
  ...
  if (mem_prof && mem_prof[0] == '1') dump_mem_prof();
}

graph_t create_graph(context_t context, const char* model_format,
                     const char* fname, ...) {
  va_list argp;
  va_start(argp, fname);
  ...
  if (model_format &&
      vload_file_model(exec_context, model_name.c_str(), model_format, fname,
                       argp) < 0) {
    if (new_context_created) delete exec_context;
  ...
  if (model_format)
    graph =
        create_graph_in_context(exec_context, graph_name, model_name.c_str());
  else
  ...
  if (new_context_created) real_graph->SetAttr(ATTR_PRIV_CONTEXT, exec_context);
...
int quant_graph(graph_t graph, int quant_mode, int node_no_quant_idxs[],
                int node_no_quant_number) {
  ...
  return quant_graph_internal(graph, quant_mode, node_no_quant_idxs,
                              node_no_quant_number);
}

int set_graph_layout(graph_t graph, int layout_type) {
  if (layout_type != TENGINE_LAYOUT_NCHW &&
      layout_type != TENGINE_LAYOUT_NHWC) {
...
graph_t merge_graph(int graph_num, graph_t graph0, graph_t graph1, ...) {
  ...
  ExecContext* exec_context =
      (ExecContext*)executor->GetExecAttr()->exec_context;
  ...
  for (int i = 1; i < graph_num; i++) {
    ...
    ExecContext* sec_context =
        (ExecContext*)sec_executor->GetExecAttr()->exec_context;
...
int destroy_graph(graph_t graph) {
  ...
  if (model_name) remove_model(model_name);
  ...
  if (real_graph->ExistAttr(ATTR_PRIV_CONTEXT)) {
    exec_context =
        any_cast<ExecContext*>(real_graph->GetAttr(ATTR_PRIV_CONTEXT));
  }
...
void release_graph_node(node_t node) {
  ...
  if (!real_node->ExistAttr(ATTR_API_GRAPH)) {
    ...
    LOG_INFO() << "node: " << real_node->GetName()
               << " do not have API Graph attribute\n";
    return;
  }
...
int set_custom_kernel(node_t node, const char* dev_name,
  ...
  if (op->GetName() == "Generic") {
    ...
    if (kernel_ops->infer_shape == nullptr ||
        strncmp(kernel_ops->op, generic_param->op_name,
                strlen(kernel_ops->op))) {
      set_tengine_errno(EINVAL);
      return -1;
    }
  ...
  if (strncmp(dev_name, ANY_DEVICE_NAME, strlen(dev_name))) {
    ...
    GraphExecutor* executor =
        any_cast<GraphExecutor*>(real_node->GetAttr(ATTR_API_GRAPH));
    ExecAttr* exec_attr = executor->GetExecAttr();
    ExecContext* exec_context =
        reinterpret_cast<ExecContext*>(exec_attr->exec_context);
  ...
  CustomKernelList* k_list =
      any_cast<CustomKernelList>(&real_node->GetAttr(ATTR_CUSTOM_KERNEL));
...
int set_node_output_tensor(node_t node, int output_idx, tensor_t tensor,
                           int tensor_type) {
  Node* real_node = reinterpret_cast<Node*>(node);
  Tensor* real_tensor = reinterpret_cast<Tensor*>(tensor);
...
int add_node_attr(node_t node, const char* attr_name, const char* type_name,
                  int size) {
...
int get_node_attr_int(node_t node, const char* attr_name, int* attr_val) {
  return get_node_attr_generic(node, attr_name, typeid(int).name(), attr_val,
                               sizeof(int));
}

int get_node_attr_float(node_t node, const char* attr_name, float* attr_val) {
  return get_node_attr_generic(node, attr_name, typeid(float).name(), attr_val,
                               sizeof(float));
}

int get_node_attr_pointer(node_t node, const char* attr_name, void* attr_val) {
  return get_node_attr_generic(node, attr_name, nullptr, attr_val,
                               sizeof(void*));
}

int get_node_attr_generic(node_t node, const char* attr_name,
                          const char* type_name, void* buf, int size) {
...
int set_node_attr_int(node_t node, const char* attr_name, const int* attr_val) {
  return set_node_attr_generic(node, attr_name, typeid(int).name(), attr_val,
                               sizeof(int));
}

int set_node_attr_float(node_t node, const char* attr_name,
                        const float* attr_val) {
  return set_node_attr_generic(node, attr_name, typeid(float).name(), attr_val,
                               sizeof(float));
}

int set_node_attr_pointer(node_t node, const char* attr_name,
                          const void* attr_val) {
  return set_node_attr_generic(node, attr_name, nullptr, attr_val,
                               sizeof(void*));
}

int set_node_attr_generic(node_t node, const char* attr_name,
                          const char* type_name, const void* buf, int size) {
  return node_set_attr_generic(node, attr_name, type_name, buf, size);
}

tensor_t create_graph_tensor(graph_t graph, const char* tensor_name,
                             int data_type) {
  GraphExecutor* executor = reinterpret_cast<GraphExecutor*>(graph);
...
tensor_t get_graph_tensor(graph_t graph, const char* tensor_name) {
  ...
  if (tensor == nullptr) set_tengine_errno(ENOENT);
...
int set_tensor_quant_param(tensor_t tensor, const float* scale,
...
int get_tensor_quant_param(tensor_t tensor, float* scale, int* zero_point,
                           int number) {
  Tensor* real_tensor = reinterpret_cast<Tensor*>(tensor);
...
int set_graph_attr(graph_t graph, const char* attr_name, const void* buf,
                   int size) {
  GraphExecutor* executor = reinterpret_cast<GraphExecutor*>(graph);
...
int run_graph(graph_t graph, int block) {
  ...
  if (executor->Run(block)) return 0;
  return -1;
}
...
int set_graph_event_hook(graph_t graph, int event, event_handler_t cb_func,
                         void* cb_arg) {
...
const char* get_node_device(node_t node) {
  ...
  const DevProposal* prop =
      any_cast<DevProposal>(&real_node->GetAttr(DEV_PROPOSAL_ATTR));
...
int get_node_dump_buffer(node_t node, void** buf, int buf_size) {
  ...
  if (ret == 0) return msg.ret_number;
...
int get_device_number(void) { return DevExecutorManager::GetNum(); }
...
int get_device_attr(const char* device_name, const char* attr_name, void* val,
                    int size) {
  Device* dev = DriverManager::GetDevice(device_name);
...
int set_device_attr(const char* device_name, const char* attr_name, void* val,
                    int size) {
  Device* dev = DriverManager::GetDevice(device_name);
...
int set_context_attr(context_t context, const char* attr_name, const void* val,
                     int val_size) {
  ExecContext* exec_context = reinterpret_cast<ExecContext*>(context);
...
int get_context_attr(context_t context, const char* attr_name, void* val,
                     int val_size) {
  ExecContext* exec_context = reinterpret_cast<ExecContext*>(context);
...
void set_log_level(enum log_level level) { SET_LOG_LEVEL((LogLevel)level); }

void set_log_output(log_print_t func) { SET_LOG_OUTPUT(func); }
